...
data "aws_ami" "RHEL" {
  ...
  filter {
    ...
    #Get the AMI details for RHEL 8 => aws ec2 describe-images --owners 309956199498 --query 'sort_by(Images, &CreationDate)[*].[CreationDate,Name,ImageId]' --filters "Name=name,Values=RHEL-8*" --region us-east-1 --output table
  }
